...
uploadRouter.post("/", upload.array("files", 30), async (req, res, next) => {
    try {
        ...
        const created = await Promise.all(uploaded.map(async (file) => {
            const relativePath = normalizeUserRelativePath(`${req.user.discordId}/${categoryFromMimeType(file.mimetype)}/${file.filename}`);
